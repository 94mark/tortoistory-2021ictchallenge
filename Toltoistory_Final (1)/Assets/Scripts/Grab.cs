...
public class Grab : MonoBehaviour
{
    ...
    private void TryGrab()
    {
        ...
        if (ARAVRInput.GetDown(ARAVRInput.Button.HandTrigger, ARAVRInput.Controller.RTouch))
        {
            ...
            if (hitObjects.Length > 0)
            {
                ...
                prevRot = OVRInput.GetLocalControllerRotation(OVRInput.Controller.RTouch);
            }
        }
    }
    ...
    private void TryUngrab()
    {
        ...
        Quaternion deltaRotation = OVRInput.GetLocalControllerRotation(OVRInput.Controller.RTouch) * Quaternion.Inverse(prevRot);
        // 이전 회전 저장
        // prevRot = ARAVRInput.RHand.rotation;
        prevRot = OVRInput.GetLocalControllerRotation(OVRInput.Controller.RTouch);
        ...
        if (ARAVRInput.GetUp(ARAVRInput.Button.HandTrigger, ARAVRInput.Controller.RTouch))
        {
            ...
            // 던지기
            //   grabbedObject.GetComponent<Rigidbody>().velocity = throwDirection * throwPower;  //PC버전
            grabbedObject.GetComponent<Rigidbody>().velocity = OVRInput.GetLocalControllerVelocity(OVRInput.Controller.RTouch) * throwPower; //오큘러스 버전
            ...
            // grabbedObject.GetComponent<Rigidbody>().angularVelocity = angularVelocity; //PC버전
            grabbedObject.GetComponent<Rigidbody>().angularVelocity = OVRInput.GetLocalControllerAngularVelocity(OVRInput.Controller.RTouch); //오큘러스 버전
    ...
    IEnumerator GrabbingAnimation()
    {
        ...
        // grabbedObject.transform.parent = ARAVRInput.RHand;  //PC버전   
        grabbedObject.transform.parent = GameObject.Find("RightControllerAnchor").transform; //오큘러스 버전
